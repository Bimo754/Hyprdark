...
| **#BUG-24** | Minimalist Monochromatic Calendar & Compact Notification Center Overhaul | High | Middle island date/time and popup had loud saturated colors, multi-window stack was too tall and overflowed screen bottom, and SwayNC had unwanted sliders/toggles | Eradicated all saturated neon colors and rainbow markup; restored Waybar middle island to pure monochromatic Apple-style typography (`󰸗 %a %b %d    %H:%M:%S`); engineered a single unified GTK3 layer-shell window (`scripts/calendar-service.py`) positioned directly below the middle island (width: 340px, max height: ~420px); styled in dark frosted glass (`rgba(22, 22, 26, 0.82)`) with hairline border; calendar on top with today highlighted in frosted circular lens; notification list directly below with Clear and dismiss buttons; acts as DBus `org.freedesktop.Notifications` daemon with non-intrusive corner toasts when closed; removed SwayNC/Dunst daemon conflicts; toggles via `Super + N`, clicking middle island, or `Escape`. Verified live with screenshots. | **Resolved** |
