...
| **#BUG-26** | Calendar Mathematical Layout, Real App Icons, and Waybar Tooltips Cleanup | High | Calendar elements felt squished and misaligned; notifications showed generic lightbulb (💡) instead of real application logos; hover tooltip box lingered above clock when opening calendar; status bar buttons displayed unwanted hover tooltips | (1) Redesigned calendar dropdown dimensions to 420x500px with generous margins, Apple typography, and centered day grid; (2) Diagnosed lightbulb bug: GTK fallback `dialog-information` renders as lightbulb in Adwaita theme, and modern apps had binary-to-desktop mismatches (e.g. `brave-browser` -> `brave-desktop`); built automatic `.desktop` file cache scanner and mapped all common aliases, replacing fallbacks with crisp symbolic notification bell; (3) Disabled clock tooltip and aligned calendar flush under clock island; (4) Set `"tooltip": false` across all Waybar modules (`custom/launcher`, `custom/target`, `clock`, `cpu`, `memory`, `pulseaudio`, `battery`, `custom/power`), strictly preserving `"tooltip": true` exclusively for `custom/vpn`; (5) Implemented deterministic PID-file signaling in `calendar-service.py` and `toggle-center-dropdown.sh`. Verified live with screenshots. | **Resolved** |
