...
| **#BUG-23** | Middle Island Redesign & Animated Calendar Dropdown | High | Middle island date/clock was a plain flat text string without clickable interaction; notifications were too large and opened on the side alongside unwanted Wi-Fi/Bluetooth tiles and sliders; users needed a cooler cyber middle island that drops down a detailed calendar with notifications directly below it | Redesigned middle island in Waybar into a sleek frosted cyber capsule with Pango dual badges (cyber blue date + neon cyan time with divider dot); engineered a dedicated, lightweight GTK layer-shell service (`hyprdark-calendar`) anchored directly below the middle island with month/year navigation, day grid, today badge, and week/day metadata; streamlined SwayNC control center to 380px centered directly below the calendar with all quick-setting buttons and sliders stripped; synchronized both cards over DBus to slide down together seamlessly via Hyprland layer-shell animation (`slide top`) on middle island click, Super+N, or Escape. Verified live with screenshots. | **Resolved** |
