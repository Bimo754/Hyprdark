...
| **#BUG-22** | Reciprocal Workspace Window Swapping & Clean Empty Sliding | High | Moving all windows unconditionally into adjacent occupied workspaces caused windows from both workspaces to collide and mix together; users needed a clean swap where windows on both workspaces exchange places, while sliding cleanly into empty workspaces | Upgraded `CTRL + SUPER + left/right` logic in `config/hypr/hyprland.lua` to bidirectional workspace swapping (`swap_workspaces`): captures `curr_wins` and `target_wins` upfront, relocates `curr_wins` to `target_id`, and simultaneously relocates `target_wins` to `curr_id` before focusing `target_id`. If `target_id` is empty, windows smoothly slide without mixing. Synchronized fallback in `config/hypr/keybinds.conf`. Verified live via REPL across both occupied and unoccupied workspaces. | **Resolved** |
| **#BUG-24** | Minimalist Monochromatic Calendar & Compact Notification Center Overhaul | High | Middle island date/time and popup had loud saturated colors, multi-window stack was too tall and overflowed screen bottom, and SwayNC had unwanted sliders/toggles | Eradicated all saturated neon colors and rainbow markup; restored Waybar middle island to pure monochromatic Apple-style typography; engineered a single unified GTK3 layer-shell window positioned directly below the middle island; styled in dark frosted glass with hairline border; calendar on top with today highlighted in frosted circular lens; notification list directly below with Clear and dismiss buttons; acts as DBus daemon with non-intrusive corner toasts. Verified live with screenshots. | **Resolved** |
| **#BUG-25** | Waybar Modularization, Island Sizing, Transparent Hover & Calendar Refactoring | High | 600-line monolithic files (`calendar-service.py`, `modules.css`), island height and color mismatch across top bar, middle island hover background unwanted, calendar box too far from bar, calendar shifted vertically when deleting notifications, notifications lacked app logos, clicking outside did not dismiss calendar, and control center button duplicated calendar toggle | (1) Decomposed `modules.css` and `calendar-service.py` into modular single-responsibility packages under 80 lines per file (`config/waybar/modules/left.css`, `center.css`, `right.css` and `scripts/calendar_service/`); (2) Standardized island `min-height: 32px` and added Hyprland `layerrule = blur, waybar` and `layerrule = ignorezero, waybar` for uniform color & height; (3) Set `#clock:hover` to 100% transparent (matching disconnected VPN); (4) Reduced calendar margin to 44px, expanded size to 390x460px, stripped header clutter, and streamlined clock to `Wed Sep 9 12:05`; (5) Built `BackdropLayer` for instantaneous dismiss on clicking anywhere outside; (6) Added `Gtk.IconTheme` app logo resolution and enlarged toasts to 360px; (7) Removed `custom/control-center` from Waybar right island; (8) Locked notifications container to fixed height eliminating all layout jumps when dismissing cards; (9) Set `col.active_border = rgba(ffffffee)` for distinct active window border. Verified live with screenshots. | **Resolved** |
| **#BUG-27** | Apple Dual-Pane Frosted Glass Calendar Overhaul & 6px Gap Removal | High | Excessive vertical gap between middle island clock and calendar card (layer-shell margin compound bug); legacy Gtk.Calendar felt cluttered, ugly, and disorganized with 42 numbers and clunky C-level GTK table borders; vertically stacked layout was tall and overwhelming | (1) Eliminated gap by adjusting `WINDOW_MARGIN_TOP = 6` (places card exactly 6px beneath Waybar island); (2) Completely eliminated legacy `Gtk.Calendar` in favor of custom `CalendarView(Gtk.Box)` built with Python `calendar` module and `Gtk.Grid`: clean `‹ Month Year ›` chevron navigation, muted single-character headers (`Su Mo Tu We Th Fr Sa`), 26x26px circular day pills showing exclusively the current month's dates, today highlighted with an authentic frosted glass specular lens, and bottom date glance; (3) Re-architected dropdown into a sleek **Dual-Pane Side-by-Side Card** (`580px × 330px`): Calendar on the left (260px), 1px hairline vertical divider, Notifications list on the right (280px); fits comfortably within 330px height in the upper 30% of the screen. Verified live with screenshots. | **Resolved** |
| **#BUG-28** | Calendar Color Harmonization, Button Reset & Fortified Notification Cards | High | Calendar UI background had a subtle navy/blue-y tint instead of pure neutral dark glass; text was dull grey; calendar buttons had unwanted GTK theme shadow borders; notification cards had unaligned icons stuck at top and lacked fortified structure | (1) Upgraded background to pure neutral Apple Dark Mode frosted glass (`rgba(18, 18, 18, 0.90)`), eradicating all blue chromatic tint; (2) Boosted typography hierarchy to crisp, high-contrast Apple white (`#ffffff` for dates, titles, app names; `rgba(255, 255, 255, 0.75)` for weekday headers; `0.85` for body text); (3) Added global GTK button reset and `set_relief(Gtk.ReliefStyle.NONE)` completely eliminating all shadow boxes and borders from calendar day buttons; (4) Fortified notification cards (`.noti-card`) with a dedicated 38x38px squircle icon badge (`.noti-icon-badge`), mathematical vertical centering of icon, text, and dismiss action, and optimized text capacity (`WINDOW_WIDTH = 600`) for seamless reading without truncation. Verified live with screenshots. | **Resolved** |
| **#BUG-29** | Notification Card Mathematical Alignment, Transparent Icon, Red Clear-All & Wi-Fi Vector Icons | High | App icon had unwanted background square; 'Notifications (n)' header was grey; Clear-All button jumped size on hover and lacked red destructive style; Program Name and body text had horizontal shift due to un-set xalign; timestamps felt detached at far right; apps without icons showed black bell placeholder; Wi-Fi/BSSID connection showed 1995-era retro bitmap | (1) Made icon badge background 100% transparent (matching unlit VPN telemetry) and omitted icon entirely when app has no icon (zero placeholder bell); (2) Restored 'NOTIFICATIONS' to solid crisp white `#ffffff` with a sleek Apple count pill badge (`.noti-count-badge`); (3) Locked Clear-All border to 1px across normal and hover states (eliminating size jump) and added authentic Apple dark red hover illumination (`rgba(255, 69, 58, 0.25)`); (4) Mathematically aligned Program Name, Title, and Description using `set_xalign(0.0)` to eliminate horizontal offset; placed time directly alongside Program Name (`Program Name · HH:MM`); (5) Mapped NetworkManager Wi-Fi/BSSID connection icons to modern vector symbolic SVGs (`network-wireless-signal-excellent-symbolic`). Verified live with screenshots. | **Resolved** |
| **#BUG-30** | Notification Center Micro-Refinements: Badge Sizing, Clear-All Compactness, ✕ Concentric Hover & Empty Centering | Medium | Number badge next to notifications text was too prominent for small counts; Clear-All button was oversized relative to header; dismiss '✕' hover pill was visibly off-center from the symbol; empty state bell and text were pushed down by legacy margin rather than geometrically centered | (1) Rescaled count badge to a delicate, understated Apple pill (`8.5px` font, `min-width: 14px; min-height: 14px; background: rgba(255, 255, 255, 0.09)`); (2) Scaled 'Clear All' button down to a compact, proportionate pill (`padding: 1px 7px; font-size: 9.5px; min-height: 16px`); (3) Built dismiss button as an exact 1:1 circle (`size_request(20, 20)`) with child label explicitly centered (`xalign=0.5, yalign=0.5, halign=CENTER, valign=CENTER`) and zero CSS padding, ensuring the `:hover` circle is perfectly concentric around the '✕'; (4) Stripped legacy top margin and enabled `vexpand=True, hexpand=True, valign=CENTER, halign=CENTER` on empty state, achieving exact vertical and horizontal centering (<0.5px deviation). Verified live with screenshots. | **Resolved** |
| **#BUG-31** | Apple-Style Stacked Notification Decks Grouped by Application | High | Notifications from the same app were listed flatly below each other rather than stacked; users requested grouping where the newest notification sits on top and older ones appear underneath as physical stacked cards with expandable deck view | (1) Engineered `NotificationStackGroup` in `scripts/calendar_service/notification_stack.py` supporting collapsed 3D card decks and expanded lists; (2) In collapsed state, top card displays newest notification with two stepped frosted glass shelves peeking underneath (`noti-stack-shelf-1` with 8px margin, `noti-stack-shelf-2` with 16px margin) and a delicate `󰅀 N more` capsule indicator pill; (3) Interactive click expands deck into full list in reverse chronological order with independent dismiss actions and `󰅃 Show less` collapse button; (4) Dismissing top card reactively surfaces the next message to top; drops cleanly to single card when down to 1 item; (5) Fixed icon resolution `TypeError` in `toast_window.py` for iconless apps. Verified live with screenshots. | **Resolved** |
| **#BUG-32** | Waybar True Circular Workspace Lenses & Stacked Notification Pill Hover Stability | High | Active workspace highlight rendered as a vertical oval (42x51px, 1.21 ratio) instead of a true circle due to container stretch; stacked notification indicator pill (`󰅀 N more`) changed shape/shrank on hover due to GTK `transition: all` layout recalculations | (1) Locked `#workspaces button` and `.active` in `config/waybar/modules/left.css` to strict 26x26px bounds (`min-width: 26px; min-height: 26px; margin: 3px 2px; -gtk-outline-radius: 999px;`), restoring mathematically verified 1:1 true circle geometry (45x45px, 1.000 ratio across all workspaces); (2) Added explicit alignment in `notification_stack.py` and locked geometry (`padding: 2px 10px; margin-top: 3px; margin-bottom: 4px; min-height: 18px; border-radius: 999px; font-size: 9px; font-weight: 600;`) on both normal and `:hover` states in `styles.css`; (3) Restricted CSS transitions to color channels (`transition: background-color 0.15s ease, border-color 0.15s ease, color 0.15s ease;`), completely eliminating shape twitching, shrinking, and layout shifts on hover. Verified live with pixel measurements and screenshots. | **Resolved** |
